...
COMP_PATH = "mr_gen/scripts/comp.wav"
HOST_PATH = "mr_gen/scripts/host.wav"
# OUTPUT_PATH = "mr_gen/scripts/result.png"
...
THRESHOLD = -4
MINIMUM_VOICED_SECTION_LENGTH = 0.3  # sec
...
def collect_utterance_section(
    ...
    while comp_index < comp_length and host_index < host_length:
        ...
        # # merge voiced sections
        # while (
        #     first_progress + comp_index < comp_length
        #     and second_progress + host_index < host_length
        # ):
        #     if first_progress + comp_index + 1 >= comp_length:
        #         break
        #     pause_length = first(first_progress + 1)[0] - first(first_progress)[1]
        #     # update second head for effective index
        #     while second(second_progress)[0] < first(first_progress)[1]:
        #         if second_progress + host_index + 1 < host_length:
        #             second_progress += 1
        #     # judge second section in pause
        #     in_pause = second(second_progress)[0] < first(first_progress + 1)[0]
        #     if in_pause:
        ...
        #     if pause_length >= int(fft_rate * pause_with_voice) and in_pause:
        #         break
        #     elif pause_length >= int(fft_rate * pause_without_voice):
        #         break
        #     else:
        #         first_progress += 1
        #         continue
...
def plot_utterance_section(
    ...
    fig.savefig(f"mr_gen/scripts/utt/utterance_{start}_{end}.png")
...
def utterance_to_turn_section(
    ...
    for utt_sec in utterance_sections:
        ...
        start = max(utt_sec[0] - warmup - mergin, 0)
...
def main():
    ...
    assert sample_rate_comp == SAMPLE_RATE
    # check length
    ...
    # compute log power
    log_power_comp = compute_log_power(waveform_comp)
    log_power_host = compute_log_power(waveform_host)
    ...
    utterance_section_comp, utterance_section_host = collect_utterance_section(
        ...
        SAMPLE_RATE / N_SHIFT,
        MINIMUM_UTTERANCE_SECTION_LENGTH,
        PAUSE_WITH_VOICE,
        PAUSE_WITHOUT_VOICE,
    )
    ...
    time_length = 15
    audio_length = len(waveform_comp) / SAMPLE_RATE
